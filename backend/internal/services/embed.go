...
// OpenAIClient talks to OpenAI's embeddings and chat completions APIs.
...
// NewOpenAIClient builds an OpenAIClient with the given API key. Extra
...
// worth of chunks.
func (c *OpenAIClient) EmbedChunks(ctx context.Context, chunks []Chunk) ([]ChunkEmbedding, error) {
	if len(chunks) == 0 {
		return nil, nil
	}
